    ...
    <el-dialog class="disabledWhite"
      ...
      <el-table
        ...
        <el-table-column
          label="加工数量">
          <template slot-scope="scope">
            <el-input v-model="scope.row.jgnumber" size="mini" clearable placeholder="请输入加工数量"></el-input>
          </template>
        </el-table-column>
        <el-table-column
          label="加工不含税单价">
          <template slot-scope="scope">
            <el-input v-model="scope.row.price" size="mini" clearable placeholder="请输入加工不含税单价"></el-input>
          </template>
        </el-table-column>
        <el-table-column
          property="sqcode"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="danger" @click="submitCg">提 交</el-button>
  ...
  methods: {
    ...
    getSqdDetail (row) {
      this.Http.get('wxsqdetaillist', {id: row.id}
        ...
        console.log(this.backDetailList)
      }).catch((error) => {
        console.log(error)
        this.$message({
          message: '服务器繁忙!',
